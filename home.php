...
function showSlides() {
  ...
  <section class="footer">
	<a href="bookingPage.php"><button type="button">BOOK NOW!</button></a>
...
$sql = "SELECT fullname, phoneNo, email FROM user_data WHERE username='$username'";
...
while($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
{
 echo '<section class="profile">
       <div class="profilePage">
         <h1>Profile</h1>
         <form>
             <label>Name : </label><div class="infop">'.$row["fullname"].'</div>
             <label>Phone Number : </label><div class="infop">'.$row["phoneNo"].'</div>
             <label>Email : </label><div class="infop">'.$row["email"].'</div><br><br>
             <a class="buttonP" href="editprofilepage.php">Edit Profile</a>
         </form>
         </div>
   </section>';
}
